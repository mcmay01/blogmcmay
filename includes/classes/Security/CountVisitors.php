...
class CountVisitors
{
    ...
    function check_last_visit() {
        ...
        $check_visit = mysqli_query($database, $check_sql);
        $check_row = mysqli_fetch_array($check_visit);
    ...
    public function mysql_result($result, $row, $field=0){
    ...
    function get_country() {
        ...
        $country_res = mysqli_query($database, $country_sql);
        $country = $this->mysql_result( $country_res, 0, "country");
        return $country;
    }
    ...
    function show_all_visits() {
        $database = mysqli_connect(DB_HOST, DB_USER, DB_PASS, DB_NAME);
        $result = mysqli_query($database,sprintf("SELECT COUNT(*) AS count FROM %s", $this->table_name));
        $visits = $this->mysql_result($result, 0, "count");
        return $visits;
    }
    function show_visits_today() {
        $database = mysqli_connect(DB_HOST, DB_USER, DB_PASS, DB_NAME);
        $res_today = mysqli_query($database, sprintf("SELECT COUNT(*) AS count FROM %s WHERE visit_date = NOW()", $this->table_name));
        $today = $this->mysql_result($res_today, 0, "count");
        return $today;
    }
    function first_last_visit($type = "last") {
        $database = mysqli_connect(DB_HOST, DB_USER, DB_PASS, DB_NAME);
        $order_dir = ($type == "last") ? "DESC" : "ASC";
        $result = mysqli_query($database, sprintf("SELECT visit_date, time FROM %s ORDER BY visit_date %s LIMIT 0,1", $this->table_name, $order_dir));
        $first_last = $this->mysql_result($result, 0, "visit_date");
        $first_last .= " ".$this->mysql_result($result, 0, "time");
        return $first_last;
    }
